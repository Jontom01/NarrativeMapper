...
def summarizedf_to_dict(df):
    final = {"subreddit": "politics", "clusters": []}

    for _, row in df.iterrows():
        mtp = row["main_talking_points"]
        #Convert to list only if it's a string representation
        if isinstance(mtp, str):
            mtp = ast.literal_eval(mtp)
        if isinstance(mtp, list) and len(mtp) > 0:
            label = mtp[:3] 
        else: 
            label = []
